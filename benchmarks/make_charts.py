...
V9_BASELINE = ("CPGC v9 (old)", 20_078_377)

# ---------------------------------------------------------------------------
# Chart 1: compressed size, CPGC v10 vs the field (horizontal bars, sorted)
# ---------------------------------------------------------------------------
entries = [("CPGC v10 -9 (this repo)", best[1], "cpgc")]
entries.append((V9_BASELINE[0], V9_BASELINE[1], "old"))
...
ax.set_title(
    "CPGC v10 levels 1-9 on enwik8 — size vs compress time",
    fontsize=11, fontweight="bold", loc="left", pad=14, color=PRIMARY,
)
ax.text(
...
if os.path.exists(e9_csv):
    ...
    entries9 = [("CPGC v10 -9 (this repo)", int(best9["comp_bytes"]), "cpgc")]
    entries9 += LTCB9
