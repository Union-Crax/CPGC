...
///
/// The worker checks it periodically: it blocks while paused, bails out when
/// cancelled, and publishes a running count of bytes processed so a UI can show
/// progress and throughput. Cheap to share across the rayon worker threads.
#[derive(Default)]
pub struct Control {
    state: AtomicU8,
    done: AtomicU64,
...
impl Control {
    ...
    /// Bytes processed so far (for progress / speed display).
...
pub fn encode(data: &[u8], level: u8) -> Vec<u8> {
    encode_framed(data, seg_size_for_level(level), &Control::new())
        .expect("uncontrolled encode is never cancelled")
}

/// Compress with a shared [`Control`] for pause/resume/cancel and progress.
/// Returns `None` if the job was cancelled before completing.
pub fn encode_with_control(data: &[u8], level: u8, ctrl: &Control) -> Option<Vec<u8>> {
    encode_framed(data, seg_size_for_level(level), ctrl)
}
...
fn encode_framed(data: &[u8], seg_size: usize, ctrl: &Control) -> Option<Vec<u8>> {
    if data.is_empty() {
        return Some(Vec::new());
    }
    ...
    let segments: Option<Vec<Vec<u8>>> = data
        .par_chunks(seg_size)
        .map(|chunk| encode_segment(chunk, ctrl))
        .collect();
...
fn decode_framed(payload: &[u8], n: usize, ctrl: &Control) -> Option<Vec<u8>> {
    ...
    let parts: Option<Vec<Vec<u8>>> = jobs
        .par_iter()
        .map(|&(p, len)| decode_segment(p, len, ctrl))
        .collect();
    let parts = parts?;
...
// How often (in bytes) the worker checks the control flag and publishes
// progress. Batching keeps the shared atomic cheap on the hot path.
const CHECK_INTERVAL: usize = 1 << 12;

/// Compress a single segment (no framing). Returns `None` if cancelled.
fn encode_segment(data: &[u8], ctrl: &Control) -> Option<Vec<u8>> {
...
fn decode_segment(payload: &[u8], n: usize, ctrl: &Control) -> Option<Vec<u8>> {
...
mod tests {
    ...
    fn roundtrip_segmented(data: &[u8], seg: usize) {
        let c = Control::new();
        let payload = encode_framed(data, seg, &c).unwrap();
    ...
    fn cancel_aborts_encode() {
        ...
        let ctrl = std::sync::Arc::new(Control::new());
        ctrl.cancel(); // cancelled before it starts
    ...
    fn segmentation_is_transparent_to_ratio() {
        ...
        let one = encode_framed(s.as_bytes(), s.len() + 1, &Control::new()).unwrap();
        let many = encode_framed(s.as_bytes(), 4096, &Control::new()).unwrap();
