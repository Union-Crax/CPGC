...
//! where the hashed tables stop being able to hold the segment's contexts.
//! Past that the window turns against you. Level 9 sits at that point, at
//! 256 MiB; see [`seg_size_for_level`].
//!
...
///
/// Measured on enwik9 with the current tables: four 64 MiB segments give
/// 48,491,970 bytes over the first 256 MB, two 128 MiB segments 47,851,150, and
/// one 256 MB segment 45,980,493 — each doubling of the window pays. Past that
...
/// segment, enwik9 comes out at 168,708,258 bytes — worse than splitting it.
///
/// So level 9 uses the largest window the tables can actually support. Raising
/// it further is a memory question, not a segmentation one: it needs
/// 2^26-bucket tables, which is about 11 GB of model.
